...
const Movimientos = () => {
  ...
  const changeCost = (productId, value) => {
    setCart((prev) =>
      prev.map((i) => (i.productId === productId ? { ...i, unitCost: value } : i)),
    );
  };
  ...
  const handleCancel = async (id) => {
    if (!confirm("¿Cancelar este movimiento? Se revertirá el efecto en el stock."))
      return;
    try {
                  ...
                  <tbody>
                    {movements.map((m) => (
                      <tr key={m.id}>
                        ...
                        <td className="text-body-sm">
                          {(m.details ?? [])
                            .map((d) => `${d.product?.name ?? d.productId} (${d.quantity})`)
                            .join(", ")}
                        </td>
                        ...
                        <td>
                          <span
                            className={`badge badge-sm rounded-full ${m.status === "Cancelado" ? "badge-error" : "badge-success"}`}
                          >
                            {m.status}
                          </span>
                        </td>
