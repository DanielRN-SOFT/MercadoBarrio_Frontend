...
const SummaryCard = ({ icon: Icon, label, value, tone = "text-primary" }) => (
  <div className="card bg-surface-container-lowest border border-outline-variant/70 rounded-2xl shadow-sm">
    <div className="card-body p-4 sm:p-5 gap-1 flex-row items-center">
      <div
        className={`w-11 h-11 rounded-xl bg-surface-container-low flex items-center justify-center shrink-0 ${tone}`}
      >
        <Icon className="text-xl" />
      </div>
      <div className="min-w-0">
        <p className="text-label-sm text-secondary">{label}</p>
        <p className="text-headline-sm font-bold text-on-surface truncate">
          {value}
        </p>
      </div>
    </div>
      ...
      {data.length === 0 ? (
        ...
        >
          ...
          >
            ...
            />
            <Tooltip
              formatter={(v) => [v, "Unidades vendidas/salidas"]}
...
const ReporteMovimientos = () => {
  ...
  const handleExport = () => {
    if (!report?.reporteCompletoPorProducto?.length) {
      addToast({
        message: "No hay datos para exportar en este período",
        type: "info",
      });
      return;
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-body-sm sm:text-body-md text-secondary">
              Identifica tus productos de mayor y menor rotación para planear el
              reabastecimiento
            </p>
          </div>
        </div>
            ...
            >
              ...
              <button
                ...
                disabled={
                  exporting ||
                  loading ||
                  !report?.reporteCompletoPorProducto?.length
                }
          ...
          <div className="card bg-surface-container-lowest border border-outline-variant/70 rounded-2xl shadow-sm">
            <div className="card-body items-center text-center py-12">
              ...
              <p className="text-body-sm text-secondary">
                No fue posible cargar el reporte. Intenta generarlo de nuevo.
              </p>
            </div>
          </div>
        ) : (
                      ...
                      <table className="table table-sm">
                        <thead>
                          <tr className="text-label-sm text-secondary">
                            ...
                            <th
                              className="cursor-pointer select-none text-right"
                              onClick={() => toggleSort("porcentajeRotacion")}
                            >
                        ...
                        <tbody>
                          {sortedRows.map((p) => (
                            <tr key={p.id}>
                              <td className="text-body-sm font-medium text-on-surface max-w-48 truncate">
                                {p.name}
                              </td>
                              <td className="text-body-sm text-right">
                                {p.entradas}
                              </td>
                              <td className="text-body-sm text-right font-semibold">
                                {p.salidas}
                              </td>
                              <td className="text-body-sm text-right text-on-surface-variant">
                                {p.porcentajeRotacion}%
                              </td>
                              <td>
                                <span
                                  className={`badge badge-sm rounded-full ${ROTATION_BADGE[p.nivelRotacion] ?? "badge-ghost"}`}
                                >
                                  {p.nivelRotacion}
                                </span>
                              </td>
                              ...
                              <td>
                                {p.requiereReabastecimiento ? (
                                  <span className="badge badge-sm badge-error rounded-full">
                                    Sí
                                  </span>
                                ) : (
